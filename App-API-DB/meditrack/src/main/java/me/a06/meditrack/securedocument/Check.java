    ...
    public static void checkJSON(JsonObject json, String client) {
        for (Map.Entry<String, JsonElement> jsonEntry : json.entrySet()) {
            try {
                ...
                if (!keysFound) {
                    System.err.println("Could not verify integrity of field " + jsonEntry.getKey() + ". The given user has no decrypting permissions.");
                }
                ...
                System.err.println("Could not verify integrity of field " + jsonEntry.getKey() + ". Issue decrypting: " + e.getMessage());
